...
def check_anketa():
    global current
    if current != ['borisovamailme@yandex.ru', 'Администратор']:
        f = open('planetsavers_anketa.json', 'r', encoding="utf8")
        f1 = f.read()
        jr = json.loads(f1)
        f.close()
...
def form1():
    ...
    '''
        ...
        current = [request.form['email'], jr[request.form['email']]["name"]]
...
def form2():
  ...
  <h1>Регистрация</h1>
    ...
    '''
        ...
        jr[request.form['email']]['class'] = request.form['class']
...
def question_form(opt):
    ...
    if current == ['войти']:
        ...
        '''
            ...
            if opt == 'ques':
                jr[text1] = {}
                jr[text1]["name"] = current[1]
                jr[text1]["ans"] = ''
                jr2[current[0]]['questions'] = str(int(jr2[current[0]]['questions']) + 1)
            else:
                try:
                    w = jr[opt]["ans"].split('!*')
                    w.append(current[1] + ':' + ' ' + text1)
                    jr[opt]["ans"] = '!*'.join(w)
                    jr2[current[0]]['answers'] = str(int(jr2[current[0]]['answers']) + 1)
                except KeyError:
                    for i in jr:
                        if opt in i:
                            opt = i
